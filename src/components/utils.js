...
import {buildCard, prependCard, checkLikeState, Card } from "./card";
    ...
    Promise.all([getUser(), getCards()])
        .then(([user, cards]) => {
            ...
            cards.forEach(card => {
                ...
                const cardMarkup = new Card({link:card.link, name:card.name}/*, '#card-template'*/)  
                
                prependCard(cardMarkup.getElement());
            })
        })
        .catch(err => {
            console.log(err);
